...
async fn handle(req: Request<Body>) -> Response<Body> {
    ...
    match (parts.method,parts.uri.path(),parts.headers.get("content-length"), parts.headers.get("content-type")) {
        ...
        )   if application_json == "application_json" => {
                ...
                if let Ok(length_u32) = length_str.parse::<u32>() {
                    if length_u32 <= 16384 {
                        if let Ok(buffer) = aggregate(stream).await {
                            ...
                            response!(status: BAD_REQUEST, body: r#"{"error":"invalid request"}"#)
                        }
                    } else {
                        // PAYLOAD_TOO_LARGE
                        response!(status: PAYLOAD_TOO_LARGE, body: r#"{"error":"request entity too large"}"#)
                    }
                } else {
                    // content-length is invalid
                    response!(status: BAD_REQUEST, body: r#"{"error":"content-length is invalid"}"#)
                }
            } else {
                // content-length is invalid
                response!(status: BAD_REQUEST, body: r#"{"error":"content-length is invalid"}"#)
