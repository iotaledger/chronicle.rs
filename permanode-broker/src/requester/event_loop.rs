...
impl EventLoop<CollectorHandle> for Requester {
    async fn event_loop(
        ...
        while let Some(event) = self.inbox.recv().await {
            match event {
                RequesterEvent::RequestFullMessage(message_id) => {
                    self.request_message_and_metadata(collector_handle, message_id).await
                }
            }
        }
        Ok(())
...
impl Requester {
    async fn request_message_and_metadata(&mut self, collector_handle: &CollectorHandle, message_id: MessageId) {
        ...
        if let (Ok(message), Ok(metadata)) = (message, metadata) {
            ...
            if let (Ok(message), Ok(metadata)) = (message, metadata) {
                ...
                if let (Ok(message), Ok(metadata)) = (message, metadata) {
                    ...
                    let full_message = FullMessage::new(message, metadata.into_data());
                    let collector_event = CollectorEvent::MessageAndMeta(self.requester_id, Some(full_message));
                    let _ = collector_handle.send(collector_event);
                    return ();
                }
            }
        }
        let collector_event = CollectorEvent::MessageAndMeta(self.requester_id, None);
        let _ = collector_handle.send(collector_event);
    }
}
