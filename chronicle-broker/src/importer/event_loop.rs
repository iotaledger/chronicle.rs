                ...
                ImporterEvent::ProcessMore(milestone_index) => {
                    ...
                    if !is_empty {
                        ...
                        for _ in 0..self.parallelism {
                            if let Some((message_id, FullMessage(message, metadata))) = iter.next() {
                                // Insert the message
                        ...
                        let logged_by = Some(self.permanode_id);
...
impl Importer {
    ...
    fn insert_message_with_metadata<I: Inherent>(
        &mut self,
        inherent_worker: &I,
    ...
    fn insert<I, K, V>(&self, inherent_worker: &I, key: K, value: V)
    where
        I: Inherent,
        K: 'static + Send + Clone,
        V: 'static + Send + Clone,
