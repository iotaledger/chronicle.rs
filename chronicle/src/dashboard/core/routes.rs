...
pub(crate) fn page_routes() -> impl Filter<Extract = impl Reply, Error = Rejection> + Clone {
    // TODO: modify the page routes after using permanode native frontend
    warp::path!("analytics" / ..)
...
pub(crate) fn ws_routes(
    users: WsUsers,
    // TODO: change the node_id to be permanode_id
    node_id: String,
    ...
    warp::path("ws")
        ...
        .map(|ws: warp::ws::Ws, users, node_id, auth_config| {
            // This will call our function if the handshake succeeds.
            ws.on_upgrade(move |socket| user_connected(socket, users, node_id, auth_config))
        })
}
...
pub(crate) fn api_routes(
    ...
    allowed_routes
        // TODO: Get the proxy address for the rest API from the config
...
pub(crate) fn routes(
    ...
    index_filter()
        .or(asset_routes())
        .or(page_routes())
        .or(ws_routes(users, node_id.clone(), auth_config.clone()))
        .or(api_routes(node_id.clone(), auth_config.clone()))
