...
/// The Websocket worker registers
pub(crate) type WsWorkers = Arc<RwLock<HashMap<String, WsWorker>>>;
...
pub(crate) async fn user_connected(ws: WebSocket, users: WsUsers, node_id: String, auth_config: DashboardAuthConfig) {
    ...
    while let Some(result) = ws_rx.next().await {
        ...
        user_message(user_id, msg, &users, &node_id, &auth_config).await;
    }
...
async fn user_message(user_id: usize, msg: Message, users: &WsUsers, node_id: &str, auth_config: &DashboardAuthConfig) {
    if !msg.is_binary() {
        return;
    }
    ...
    if let Some(user) = users.write().await.get_mut(&user_id) {
        match command {
            WsCommand::Register => {
                ...
                send_init_values(&topic, &user).await;
...
// TODO: Currently need to send fake data to the frontend
async fn send_init_values(topic: &WsTopic, user: &WsUser) {
    match topic {
        WsTopic::SyncStatus => {
            let event = WsEvent::new(
                WsTopic::SyncStatus,
                WsEventInner::SyncStatus(SyncStatusResponse { lmi: 99999, cmi: 99999 }),
            );
            user.send(event);
        }
