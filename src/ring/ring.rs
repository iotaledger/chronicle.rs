...
pub fn build_ring(nodes: &Nodes, registry: Registry,reporter_count: u8, version: u8) -> (Arc<GlobalRing>, Box<Weak<GlobalRing>>) {
    ...
    // create arc_ring
    let arc_ring = Arc::new((Uniform::new(0,reporter_count),version ,registry, root_vnode));
